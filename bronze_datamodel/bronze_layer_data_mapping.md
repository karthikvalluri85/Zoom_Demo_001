...
- Primary key uniqueness enforced at table level
- Duplicate key handling: REJECT and log errors

### Referential Integrity
- Foreign key relationships preserved from source
- Orphaned records flagged for investigation
- Cross-reference validation between related tables

### Data Quality Checks
1. **Null Value Validation**
   - Critical fields (IDs, timestamps) cannot be null
   - Optional fields allow null values

2. **Data Format Validation**
   - Email format validation for EMAIL fields
   - Timestamp format consistency checks
   - Numeric range validation for amounts and counts

3. **Business Rule Validation**
   - Start time must be before end time for meetings/webinars
   - Join time must be before leave time for participants
   - License start date must be before end date

### Error Handling
- Invalid records logged to error table
- Data quality metrics tracked and reported
- Failed loads trigger alerts for investigation
...
### Ingestion Strategy
1. **Full Load**: Initial historical data load
2. **Incremental Load**: Delta changes based on UPDATE_TIMESTAMP
3. **Real-time Streaming**: For high-frequency events (optional)

### Data Preservation Principles
- No data transformations applied in Bronze layer
- Original data structure maintained
- All source columns preserved
- Metadata enhanced for governance

### Load Frequency
- **Users**: Daily batch load
- **Meetings**: Real-time streaming
- **Participants**: Real-time streaming
- **Webinars**: Daily batch load
- **Licenses**: Weekly batch load
- **Support Tickets**: Daily batch load
- **Billing Events**: Daily batch load
- **Feature Usage**: Real-time streaming
